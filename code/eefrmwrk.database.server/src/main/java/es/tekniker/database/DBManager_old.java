...
public class DBManager_old {
	...
	private static final Log log = LogFactory.getLog(DBManager_old.class);
	 ...
	 * 
	 */
	public static List<ValueRead> readVR(String varName, Long start, Long end) throws BaseException{
		List<ValueRead> vrList = new ArrayList<ValueRead>();
			...
			if (vmd == null) {
				...
				if (bd.equals(VarMetadata.Cassandra)) {
					...
					for (HColumn<Long, Object> v : l) {
						ValueRead vr = new ValueRead(vmd.getVmdId(),v.getValue() + "", new Date(v.getName()));
						vrList.add(vr);
					}
				}else if (bd.equals(VarMetadata.PostgreSQL))
					vrList = ValueReadManager.getValues(vmd.getVmdId(), start,end, maxResults, true,session);	
				else 
	...
	public static ValueRead getLastVR (String varName,Long date) throws BaseException{
		ValueRead vr = null;
		Session session= null;
		try {
			log.debug("Opening session");
	...
	public static ValueRead getLastVR (String varName,Long date,Session session) throws BaseException{
			...
			if (bd.equals(VarMetadata.Cassandra)) {
				...
				List<HColumn<Long, Object>> l = HectorFactory.getInstance()
						.getValues(
								DatabaseProperties.getProperty("NoSQL_VRColFam"),//"ValueRead",
								vmd.getVmdId(), 
								1,
								Long.MIN_VALUE, 
								date - 1, 
								true);
				if (l.size()>0)
				vr = new ValueRead(vmd.getVmdId(), l.get(l.size() - 1).getValue() + "",
						new Date(l.get(l.size() - 1).getName()));}
				catch(Exception e){
			...
			if (bd.equals(VarMetadata.PostgreSQL)) {
				List<ValueRead> vrList = ValueReadManager.getValues(vmd.getVmdId(),new Long(0), date-1,1, false,session);
				if(vrList.size()>0)
					vr = vrList.get(0);
			}	
		return vr;
	...
	public static void saveVR(String varName, String varValue, Date varTimestamp,Session session) throws Exception {
				...
				ValueRead vr = new ValueRead(vmd.getId(), varValue, varTimestamp);
				String bd = vmd.getVmdStoredbd().trim();
				if (bd.equals(VarMetadata.Cassandra)) {
					HectorFactory.getInstance().updateCol(
							DatabaseProperties.getProperty("NoSQL_VRColFam"),
							vmd.getVmdId(), vr.getVrTimestamp().getTime(),
							vr.getVrValue());
				} else if (bd.equals(VarMetadata.PostgreSQL)) {
					ValueReadManager.save(vr,session);
				} else {
					throw new BaseException("DBManager_SAVEVR2","Unknown storage DB "+bd);	
				}	
	}	
	
	public static void saveVR(String varName, String varValue, Date varTimestamp) throws BaseException {
		Session session= null;
		try {
				log.debug("Opening session");
				session = HibernateUtil.currentSession();
	...
	public static long saveOPCNode(String url, String nodeName, String nodeNmspc, String value, Date timestamp) throws BaseException {
		Session session= null;
		Long subId=(long)-1;
		try {
				log.debug("Opening session");
				...
				ValueRead vr = new ValueRead(vmd.getVmdId(),value,timestamp);
	 ...
	 */
	public static String deleteVariableValues(String varName) {
		try {
			...
			if (vmd.getVmdStoredbd().equals(VarMetadata.PostgreSQL)) {
				// BORRAMOS VALORES
				ValueReadManager.deleteByVarMetadata(vmd.getId());
			}
			if (vmd.getVmdStoredbd().equals(VarMetadata.Cassandra)) {
				...
				for (HColumn<Long, Object> v : l) {
					HectorFactory.getInstance().updateCol("H_ValueRead",vmd.getVmdId(), v.getName(), v.getValue());
							
				}
				// BORRAMOS VALORES
				HectorFactory.getInstance().deleteCol("ValueRead",vmd.getVmdId());
			}
	 ...
	 * 
	 * Device dev= DeviceManager.findByName(devName); HashMap<Long, ValueRead>
	 * varMap = new HashMap<Long, ValueRead>(); HashMap<Long, VarMetadata>
	 * vmdMap = new HashMap<Long, VarMetadata>(); List<ValueRead> vrList = new
	 * ArrayList<ValueRead>();
	 * 
	 * for (VarMetadata v : VarMetadataManager.findByDevice(dev.getDevId())) {
	 * vmdMap.put(v.getVmdId(),v); List<ValueRead> l = read(v.getVmdId(), start,
	 ...
	 * 
	 * Collections.sort(vrList, new ValueRead.ValueRead_DateComparator()); for
	 * (ValueRead vr : vrList) {
	...
	public static void saveGEM(long id, long timestamp, Object value)
	  ...
	  public static void main(String[] args) throws Exception { 
		 ...
		 DBManager_old.deleteVariableValues("var1");
		for( ValueRead v: DBManager_old.readVR("var1", Long.MIN_VALUE, Long.MAX_VALUE)){
